...
//
//	url := fmt.Sprintf("/users/%s/security/personal_api_keys", s.DefaultUser.UID)
//
...
//	body := serialize(bodyStr)
//
//	url := fmt.Sprintf("/organisations/projects/%s/security/endpoints/%s/keys", s.DefaultProject.UID, endpoint.UID)
//
//	req := createRequest(http.MethodPost, url, "", body)
...
//	require.NoError(s.T(), err)
//
//	url := fmt.Sprintf("/organisations/%s/projects/%s/security/keys/regenerate", s.DefaultOrg.UID, s.DefaultProject.UID)
//
...
//	body := serialize(bodyStr)
//
//	url := fmt.Sprintf("/organisations/security/endpoints/%s/keys?projectID=%s", endpoint.UID, s.DefaultProject.UID)
//
//	req := createRequest(http.MethodPost, url, "", body)
//	req.Header.Set("Authorization", fmt.Sprintf("BEARER %s", keyString)) // authenticate with previously generated key
//	w := httptest.NewRecorder()
...
//
//	url := fmt.Sprintf("/users/%s/security/personal_api_keys/%s/revoke", s.DefaultUser.UID, apiKey.UID)
//
//	req := createRequest(http.MethodPut, url, "", nil)
//	err := s.AuthenticatorFn(req, s.Router)
//	require.NoError(s.T(), err)
...
//
//	url := fmt.Sprintf("/users/%s/security/personal_api_keys?keyType=personal_key", s.DefaultUser.UID)
...
//
//	url := fmt.Sprintf("/organisations/%s/projects/%s/endpoints/%s/keys", s.DefaultOrg.UID, s.DefaultProject.UID, endpoint.UID)
//	req := createRequest(http.MethodGet, url, "", nil)
//	err := s.AuthenticatorFn(req, s.Router)
//	require.NoError(s.T(), err)
//
...
//
//	url := fmt.Sprintf("/organisations/%s/projects/%s/endpoints/%s/keys/%s/revoke", s.DefaultOrg.UID, s.DefaultProject.UID, endpoint.UID, apiKey.UID)
//	req := createRequest(http.MethodPut, url, "", nil)
//	err := s.AuthenticatorFn(req, s.Router)
//	require.NoError(s.T(), err)
//
