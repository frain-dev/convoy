...
func (u *AuthIntegrationTestSuite) SetupTest() {
	...
	apiRepo := api_keys.New(u.ConvoyApp.A.Logger, u.ConvoyApp.A.DB)
	userRepo := users.New(u.ConvoyApp.A.Logger, u.ConvoyApp.A.DB)
...
func (s *DashboardIntegrationTestSuite) TestGetDashboardSummary() {
	...
	eventDelivery := event_deliveries.New(log.New("convoy", slog.LevelError), s.ConvoyApp.A.DB)
...
func (s *SourceIntegrationTestSuite) Test_GetSourceBy_ValidSource() {
	...
	sourceRepo := sources.New(log.New("convoy", slog.LevelError), s.ConvoyApp.A.DB)
...
func (u *UserIntegrationTestSuite) Test_RegisterUser() {
	...
	dbUser, err := users.New(u.ConvoyApp.A.Logger, u.ConvoyApp.A.DB).FindUserByID(context.Background(), response.UID)
	require.NoError(u.T(), err)
