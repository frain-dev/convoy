...
func (s *PublicEventIntegrationTestSuite) Test_GetEventDeliveriesPaged() {
	...
	subscription2, err := testdb.SeedSubscription(s.ConvoyApp.A.DB, s.DefaultProject, ulid.Make().String(), datastore.OutgoingProject, &datastore.Source{}, endpoint2, &datastore.RetryConfiguration{}, &datastore.AlertConfiguration{}, &datastore.FilterConfiguration{
		EventTypes: []string{"*"},
		Filter:     datastore.FilterSchema{Headers: datastore.M{}, Body: datastore.M{}},
	})
	require.NoError(s.T(), err)
	...
	_, err = testdb.SeedEventDelivery(s.ConvoyApp.A.DB, event2, endpoint2, s.DefaultProject.UID, ulid.Make().String(), datastore.FailureEventStatus, subscription2)
	require.NoError(s.T(), err)
...
func (s *PublicProjectIntegrationTestSuite) TestDeleteProjectWithPersonalAPIKey() {
	...
	projectRepo := projects.New(log.New("convoy", slog.LevelInfo), s.ConvoyApp.A.DB)
...
func (s *PublicSubscriptionIntegrationTestSuite) Test_CreateSubscription() {
	...
	subRepo := subscriptions.New(log.New("convoy", slog.LevelInfo), s.ConvoyApp.A.DB)
...
func (s *PublicEventTypeIntegrationTestSuite) SetupTest() {
	...
	apiKeyRepo := api_keys.New(s.ConvoyApp.A.Logger, s.ConvoyApp.A.DB)
	userRepo := users.New(s.ConvoyApp.A.Logger, s.ConvoyApp.A.DB)
	portalLinkRepo := portal_links.New(s.ConvoyApp.A.Logger, s.ConvoyApp.A.DB)
