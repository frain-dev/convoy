...
func (h *Handler) ProcessOrganisationMemberInvite(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		h.A.Logger.Error("failed to process accepted url query", "error", err)
...
func (h *Handler) FindUserByInviteToken(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		h.A.Logger.Error("failed to find user by invite token", "error", err)
		_ = render.Render(w, r, util.NewServiceErrResponse(err))
		return
	}
...
func (h *Handler) ResendOrganizationInvite(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		h.A.Logger.Error("failed to resend organisation member invite", "error", err)
		_ = render.Render(w, r, util.NewServiceErrResponse(err))
		return
	}
...
func (h *Handler) CancelOrganizationInvite(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		h.A.Logger.Error("failed to cancel organisation member invite", "error", err)
		_ = render.Render(w, r, util.NewServiceErrResponse(err))
		return
	}
