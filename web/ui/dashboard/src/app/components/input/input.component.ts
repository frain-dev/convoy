...
@Directive({
	...
	standalone: true,
	host: {
		class: 'transition-all duration-[.3s] w-full font-normal text-14 placeholder:text-new.gray-300 text-new.gray-600 border border-new.primary-25 valid:border-new.primary-25 disabled:text-new.gray-400 disabled:border-new.primary-25 disabled:bg-white-100 hover:border-new.primary-25 focus:border-new.primary-300 focus:bg-white-100 outline-none rounded-4px placeholder:opacity-[.48] bg-white-100 py-12px px-16px appearance-none',
...
@Component({
	...
	host: {
		class: 'w-full font-normal text-12 text-new.gray-400 mb-8px flex items-center justify-between'
	},
	template: `
		...
		<span *ngIf="required === 'true'" class="text-10 text-new.gray-400 px-1 font-medium">required</span>
	`,
