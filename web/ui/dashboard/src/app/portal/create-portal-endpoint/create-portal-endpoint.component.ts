    ...
    async ngOnInit() {
        ...
        if (this.action === 'create' && this.projectType === 'outgoing' && this.selectedEventTypes.length === 0 && this.eventTypes.length > 0) {
            ...
            if (wildcardExists) {
                ...
                // Fall back to the first event type if the wildcard doesn't exist
        ...
        if (this.selectedEventTypes.filter(it => it !== "*").length === 0) {
        ...
        try {
            ...
            if (this.subscriptionId == 'new' || !(this.isUpdateAction || this.editMode)) {
                ...
                try {
                    ...
                    // Handle filters update for the existing subscription
    ...
    private async updateFiltersForSubscription(): Promise<void> {
        ...
        try {
            ...
            if (filtersToDelete.length > 0) {
                // Note: this is handled on the backend already, no need to do anything.
