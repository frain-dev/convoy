...
import { endpointsService, getEndpoint } from '@/services/endpoints.service';
...
// FIXME Inconsistent state  after updating endpoint
// Update an endpoint field, e.g owner_id, and save.
// Click the updated endpoint in the endpoints list to edit again.
// The owner_id field shown on the form is the previous value.

export const Route = createFileRoute(
	'/projects_/$projectId/endpoints/$endpointId',
)({
	component: UpdateEndpointPage,
	loader: async ({ params }) => {
		const { data: endpoint } = await getEndpoint(params.endpointId);
		const perms = await authService.getUserPermissions();
		return { endpoint, canManageEndpoints: perms.includes('Endpoints|MANAGE') };
	},
});
...
function UpdateEndpointPage() {
	const { endpoint, canManageEndpoints } = Route.useLoaderData();
	...
	const updateEndpoint = async (formData: EndpointFormValues) => {
		...
		try {
			// Create new endpoint
			const response = await endpointsService.updateEndpoint(params.endpointId, endpointValue);
			// toast.success(response.message || 'Endpoint updated successfully');
	...
	const form = useForm<EndpointFormValues>({
		...
		defaultValues: {
			name: endpoint.name,
			url: endpoint.url,
			support_email: endpoint.support_email,
			slack_webhook_url: endpoint.slack_webhook_url,
			secret: endpoint.secrets?.at(endpoint.secrets.length - 1)?.value ?? null,
			...
			rate_limit_duration: endpoint.rate_limit_duration
				? Number(endpoint.rate_limit_duration)
				: null,
			authentication: {
				type: 'api_key',
				api_key: {
					header_name: endpoint.authentication?.api_key?.header_name ?? '',
					header_value: endpoint.authentication?.api_key?.header_value ?? '',
				},
			},
		...
		<DashboardLayout showSidebar={false}>
				<section className="flex flex-col p-2 max-w-[770px] min-w-[600px] w-full m-auto my-4">
					<div className="flex justify-start items-center gap-2">
						...
						<h1 className="font-semibold text-sm">Edit Endpoint</h1>
					</div>
					...
					<div className="border border-new.primary-50 rounded-8px p-6 relative w-full">
						<Form {...form}>
							<form
								onSubmit={form.handleSubmit(updateEndpoint)}
								className="w-full"
							>
								...
								<div className="w-full">
												...
												<div className="flex items-center">
													...
													<TooltipProvider>
														<Tooltip>
															...
															<TooltipContent>
																You can set your provided endpoint
																authentication if any is required
															</TooltipContent>
														</Tooltip>
													</TooltipProvider>
												</div>
									...
									{showConfig('signature') && (
										<div className="border-l-2 border-primary-100 pl-4 mb-10 w-full">
											...
											<RadioGroup
												defaultValue={
													form.watch('advanced_signatures') ? 'false' : 'true'
												}
											...
											>
												<div className="flex items-start">
													<label
														className={cn(
															...
															form.watch('advanced_signatures') === false
												...
												<div className="flex items-start">
													<label
														className={cn(
															'cursor-pointer border w-full transition-all ease-in duration-200 flex items-start gap-x-2 p-4 rounded-sm',
															form.watch('advanced_signatures') === true
																? 'border-new.primary-300 bg-[#FAFAFE]'
																: 'border-neutral-5',
														)}
													>
								...
								<div className="flex justify-end mt-10 mb-4 w-full">
									<Button
										type="submit"
										disabled={isUpdating || !canManageEndpoints}
										variant="ghost"
