...
- **Webhook signature verification stays hand-written.** Generators do not own crypto. `src/webhook.ts` and the shared `tests/signature-vectors.json` contract remain the source of truth for verify (see `.speakeasyignore`).
