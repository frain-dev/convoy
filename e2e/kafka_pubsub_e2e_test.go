...
func TestE2E_Kafka_Single_BasicDelivery(t *testing.T) {
	env := SetupE2EWithKafka(t)
	...
	// Create Kafka topic and source
	topicName := "test-topic-" + ulid.Make().String()
	consumerGroupID := "test-cg-" + ulid.Make().String()
	CreateKafkaTopic(t, env.KafkaBroker, topicName, 1, 1)
	source := CreateKafkaSource(
		t, db, env.ctx, env.Project,
		[]string{env.KafkaBroker}, topicName, consumerGroupID, 1, nil, nil,
	)
	...
	// Sync sources to pick up the new Kafka source
	env.SyncSources(t)
	...
	PublishSingleKafkaMessage(
		t, env.KafkaBroker, topicName,
...
func TestE2E_Kafka_Fanout_MultipleEndpoints(t *testing.T) {
	env := SetupE2EWithKafka(t)
	...
	// Publish fanout Kafka message
	data := map[string]interface{}{
		"amount":      100,
		"customer_id": "cust-" + ulid.Make().String(),
	}
	PublishFanoutKafkaMessage(
		t, env.KafkaBroker, topicName,
...
func TestE2E_Kafka_Broadcast_AllSubscribers(t *testing.T) {
	env := SetupE2EWithKafka(t)
	...
	// Publish broadcast Kafka message
	data := map[string]interface{}{
		"severity": "high",
		"message":  "System maintenance scheduled",
		"alert_id": "alert-" + ulid.Make().String(),
	}
	PublishBroadcastKafkaMessage(
		t, env.KafkaBroker, topicName,
		eventType, data,
	)
...
func TestE2E_Kafka_Single_EventTypeFilter(t *testing.T) {
	env := SetupE2EWithKafka(t)
...
func TestE2E_Kafka_Single_WildcardEventType(t *testing.T) {
	env := SetupE2EWithKafka(t)
...
func TestE2E_Kafka_Fanout_EventTypeFilter(t *testing.T) {
	env := SetupE2EWithKafka(t)
...
func TestE2E_Kafka_Broadcast_EventTypeFilter(t *testing.T) {
	env := SetupE2EWithKafka(t)
	...
	// Create Kafka topic and source first
	topicName := "test-topic-" + ulid.Make().String()
	consumerGroupID := "test-cg-" + ulid.Make().String()
	CreateKafkaTopic(t, env.KafkaBroker, topicName, 1, 1)
	source := CreateKafkaSource(
...
func TestE2E_Kafka_Single_BodyFilter_Equal(t *testing.T) {
	env := SetupE2EWithKafka(t)
...
func TestE2E_Kafka_Single_BodyFilter_GreaterThan(t *testing.T) {
	env := SetupE2EWithKafka(t)
...
func TestE2E_Kafka_Single_BodyFilter_In(t *testing.T) {
	env := SetupE2EWithKafka(t)
...
func TestE2E_Kafka_Single_HeaderFilter(t *testing.T) {
	env := SetupE2EWithKafka(t)
	...
	PublishSingleKafkaMessageWithHeaders(
		t, env.KafkaBroker, topicName,
...
func TestE2E_Kafka_Single_CombinedFilters(t *testing.T) {
	env := SetupE2EWithKafka(t)
...
func TestE2E_Kafka_Single_SourceBodyTransform(t *testing.T) {
	env := SetupE2EWithKafka(t)
	...
	// Create Kafka topic and source with body transformation
	topicName := "test-topic-" + ulid.Make().String()
	consumerGroupID := "test-cg-" + ulid.Make().String()
	CreateKafkaTopic(t, env.KafkaBroker, topicName, 1, 1)
	source := CreateKafkaSource(
		t, db, env.ctx, env.Project,
		[]string{env.KafkaBroker}, topicName, consumerGroupID, 1, &bodyFunction, nil,
	)
...
func TestE2E_Kafka_Single_SourceHeaderTransform(t *testing.T) {
	env := SetupE2EWithKafka(t)
	...
	// Create Kafka topic and source with header transformation
	topicName := "test-topic-" + ulid.Make().String()
	consumerGroupID := "test-cg-" + ulid.Make().String()
	CreateKafkaTopic(t, env.KafkaBroker, topicName, 1, 1)
	source := CreateKafkaSource(
		t, db, env.ctx, env.Project,
		[]string{env.KafkaBroker}, topicName, consumerGroupID, 1, nil, &headerFunction,
	)
	require.NotNil(t, source)
...
func TestE2E_Kafka_Single_NoMatchingSubscription(t *testing.T) {
	env := SetupE2EWithKafka(t)
...
func TestE2E_Kafka_Single_InvalidEndpoint(t *testing.T) {
	env := SetupE2EWithKafka(t)
...
func TestE2E_Kafka_Single_MissingEventType(t *testing.T) {
	env := SetupE2EWithKafka(t)
	...
	// Manually publish raw JSON to Kafka
...
func TestE2E_Kafka_Single_MalformedPayload(t *testing.T) {
	env := SetupE2EWithKafka(t)
	...
	require.NoError(t, err)
	t.Log("Published malformed JSON to topic")
...
func TestE2E_Kafka_Fanout_MissingOwnerID(t *testing.T) {
	env := SetupE2EWithKafka(t)
...
func TestE2E_Kafka_Single_FilterMismatch(t *testing.T) {
	env := SetupE2EWithKafka(t)
...
func TestE2E_Kafka_Single_MultipleWorkers(t *testing.T) {
	env := SetupE2EWithKafka(t)
	...
	// Create Kafka topic and source with 3 workers for concurrent processing
	topicName := "test-topic-" + ulid.Make().String()
	consumerGroupID := "test-cg-" + ulid.Make().String()
	CreateKafkaTopic(t, env.KafkaBroker, topicName, 1, 1)
	source := CreateKafkaSource(
		t, db, env.ctx, env.Project,
		[]string{env.KafkaBroker}, topicName, consumerGroupID, 3, nil, nil, // 3 workers
	)
	require.NotNil(t, source)
...
// Helper function for publishing Kafka messages with custom headers (for header filtering)
func PublishSingleKafkaMessageWithHeaders(t *testing.T, broker string, topic string,
	endpointID string, eventType string, data map[string]interface{}, customHeaders map[string]string) {
