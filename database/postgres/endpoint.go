...
const (
	createEndpoint = `
            INSERT INTO convoy.endpoints (
                ...
                mtls_client_cert, mtls_client_cert_cipher, oauth2_config, oauth2_config_cipher,
                basic_auth_config, basic_auth_config_cipher, content_type
            )
            VALUES
              (
               ...
               CASE WHEN $19 THEN pgp_sym_encrypt($22::TEXT, $20) END,
               CASE WHEN $19 THEN NULL ELSE $23::jsonb END,
               CASE WHEN $19 THEN pgp_sym_encrypt($23::TEXT, $20) END, $24
              );
            `
    ...
    END AS oauth2_config,
	CASE
        WHEN e.is_encrypted THEN pgp_sym_decrypt(e.basic_auth_config_cipher::bytea, $3)::jsonb
        ELSE e.basic_auth_config
    END AS basic_auth_config,
	e.content_type
	...
	basic_auth_config_cipher = CASE
        WHEN is_encrypted THEN pgp_sym_encrypt($21::TEXT, $18)
    END,
	updated_at = NOW(), content_type = $22
...
func (e *endpointRepo) CreateEndpoint(ctx context.Context, endpoint *datastore.Endpoint, projectID string) error {
	...
	// Get BasicAuth config if authentication type is BasicAuth
	var basicAuthConfig *datastore.BasicAuth
	if endpoint.Authentication != nil && endpoint.Authentication.Type == datastore.BasicAuthentication {
		basicAuthConfig = endpoint.Authentication.BasicAuth
	}

	var apiKeyHeaderName, apiKeyHeaderValue string
	if ac.ApiKey != nil {
		apiKeyHeaderName = ac.ApiKey.HeaderName
		apiKeyHeaderValue = ac.ApiKey.HeaderValue
	}
	...
	args := []interface{}{
		...
		projectID, ac.Type, apiKeyHeaderName, apiKeyHeaderValue, isEncrypted, key,
		endpoint.MtlsClientCert, oauth2Config, basicAuthConfig, contentType,
	}
...
func (e *endpointRepo) FindEndpointByID(ctx context.Context, id, projectID string) (*datastore.Endpoint, error) {
	...
	// Map BasicAuthConfig to Authentication.BasicAuth if present
	if endpoint.BasicAuthConfig != nil {
		if endpoint.Authentication == nil {
			endpoint.Authentication = &datastore.EndpointAuthentication{
				Type: datastore.BasicAuthentication,
			}
		} else {
			endpoint.Authentication.Type = datastore.BasicAuthentication
		}
		endpoint.Authentication.BasicAuth = endpoint.BasicAuthConfig
		endpoint.BasicAuthConfig = nil
	}
...
func (e *endpointRepo) UpdateEndpoint(ctx context.Context, endpoint *datastore.Endpoint, projectID string) error {
	...
	r, err := e.db.GetReadDB().ExecContext(ctx, updateEndpoint, endpoint.UID, projectID, endpoint.Name, endpoint.Status, endpoint.OwnerID, endpoint.Url,
		...
		ac.Type, apiKeyHeaderName, apiKeyHeaderValue, endpoint.Secrets, key,
		endpoint.MtlsClientCert, oauth2Config, basicAuthConfig, contentType,
	)
	if err != nil {
		isEncErr, err2 := e.isEncryptionError(err)
