...
func (p *Postgres) Collect(ch chan<- prometheus.Metric) {
	...
	if cachedMetrics != nil && lastRun.Add(time.Duration(metricsConfig.Prometheus.SampleTime)*time.Second).After(now) {
		...
		if err != nil {
			slog.Error(fmt.Sprintf("Failed to collect metrics data: %v", err))
			if cachedMetrics != nil {
				metrics = cachedMetrics
				slog.Warn("Using cached metrics due to collection failure")
			} else {
	...
	for _, metric := range metrics.EventQueueMetrics {
		...
		if _, ok := metricsMap[key]; ok {
			slog.Warn(fmt.Sprintf("Duplicate metric detected and skipped: event_queue_total (project: %s, source: %s)", metric.ProjectID, metric.SourceId))
			continue
		}
	...
	for _, metric := range metrics.EventQueueBacklogMetrics {
		...
		if _, ok := metricsMap[key]; ok {
			slog.Warn(fmt.Sprintf("Duplicate metric detected and skipped: event_queue_backlog (project: %s, source: %s)", metric.ProjectID, metric.SourceId))
			continue
		}
		ch <- prometheus.MustNewConstMetric(
	...
	for _, metric := range metrics.EventDeliveryQueueMetrics {
		...
		if _, ok := metricsMap[key]; ok {
			slog.Warn(fmt.Sprintf("Duplicate metric detected and skipped: event_delivery_queue_total (project: %s, endpoint: %s, status: %s, event_type: %s, source: %s, organisation_id: %s)", metric.ProjectID, metric.EndpointId, metric.Status, metric.EventType, metric.SourceId, metric.OrganisationID))
			continue
		}
		ch <- prometheus.MustNewConstMetric(
	...
	for _, metric := range metrics.EventQueueEndpointBacklogMetrics {
		...
		if _, ok := metricsMap[key]; ok {
			slog.Warn(fmt.Sprintf("Duplicate metric detected and skipped: event_delivery_queue_backlog (project: %s, endpoint: %s, source: %s)", metric.ProjectID, metric.EndpointId, metric.SourceId))
			continue
		}
		ch <- prometheus.MustNewConstMetric(
	...
	for _, metric := range metrics.EventQueueEndpointAttemptMetrics {
		...
		if _, ok := metricsMap[key]; ok {
			slog.Warn(fmt.Sprintf("Duplicate metric detected and skipped: event_delivery_attempts_total (project: %s, endpoint: %s, status: %s, status_code: %s)", metric.ProjectID, metric.EndpointId, metric.Status, metric.StatusCode))
			continue
		}
		ch <- prometheus.MustNewConstMetric(
...
func (p *Postgres) materializedViewExists(ctx context.Context, viewName string) bool {
	...
	if err != nil {
		slog.Warn(fmt.Sprintf("Failed to check if materialized view %s exists: %v", viewName, err))
		return false
	}
	return exists
}
