...
func Test_FindEndpointsByID(t *testing.T) {
	...
	for i := 0; i < 7; i++ {
		...
		err := newEndpointRepo.CreateEndpoint(context.Background(), endpoint, project.UID)
		require.NoError(t, err)

		event := generateEvent(t, db)
		event.Endpoints = []string{endpoint.UID}
		require.NoError(t, newEventRepo.CreateEvent(context.Background(), event))
	}

	emptyEndpoints, err := newEndpointRepo.FindEndpointsByID(context.Background(), ids, "")
	require.NoError(t, err)
	require.Equal(t, 0, len(emptyEndpoints))

	dbEndpoints, err := newEndpointRepo.FindEndpointsByID(context.Background(), ids, project.UID)
	require.NoError(t, err)
