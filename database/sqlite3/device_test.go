...
func Test_CreateDevice(t *testing.T) {
	...
	newDevice, err := repo.FetchDeviceByID(context.Background(), device.UID, device.EndpointID, device.ProjectID)
...
func Test_UpdateDevice(t *testing.T) {
	...
	err := repo.UpdateDevice(context.Background(), device, device.EndpointID, device.ProjectID)
	require.NoError(t, err)
...
func Test_DeleteDevice(t *testing.T) {
	...
	err := repo.DeleteDevice(context.Background(), device.UID, device.EndpointID, device.ProjectID)
	require.NoError(t, err)

	_, err = repo.FetchDeviceByID(context.Background(), device.UID, device.EndpointID, device.ProjectID)
...
func Test_LoadDevicesPaged(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			_, pageable, err := repo.LoadDevicesPaged(context.Background(), project.UID, &datastore.ApiKeyFilter{}, tc.pageData)
			require.NoError(t, err)
