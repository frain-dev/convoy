...
const (
	createProject = `
	INSERT INTO projects (id, name, type, logo_url, organisation_id, project_configuration_id)
	...
	UPDATE project_configurations SET
	...
	SELECT
		...
		p.deleted_at
	FROM projects p
	LEFT JOIN project_configurations c
...
`
 ...
 `
	...
	projectStatistics = `
	SELECT
	(SELECT COUNT(*) FROM subscriptions WHERE project_id = $1 AND deleted_at IS NULL) AS total_subscriptions,
	(SELECT COUNT(*) FROM endpoints WHERE project_id = $1 AND deleted_at IS NULL) AS total_endpoints,
	(SELECT COUNT(*) FROM sources WHERE project_id = $1 AND deleted_at IS NULL) AS total_sources,
	(SELECT COUNT(*) FROM events WHERE project_id = $1 AND deleted_at IS NULL) AS messages_sent;
	`

	updateProjectEndpointStatus = `
	UPDATE endpoints SET status = ?, updated_at = NOW()
