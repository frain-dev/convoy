...
const (
	...
	INSERT INTO organisation_invites (id, organisation_id, invitee_email, token, role_type, role_project, role_endpoint, status, expires_at)
	...
	UPDATE organisation_invites SET
	deleted_at = NOW()
	WHERE id = $1 AND deleted_at IS NULL;
	`
)
