...
const (
	createEventDelivery = `
    INSERT INTO event_deliveries (id,project_id,event_id,endpoint_id,device_id,subscription_id,headers,status,metadata,cli_metadata,description,url_query_params,idempotency_key,event_type,acknowledged_at)
	...
	LEFT JOIN endpoints ep ON ed.endpoint_id = ep.id
	LEFT JOIN events ev ON ed.event_id = ev.id
    ...
    SELECT COUNT(id) FROM event_deliveries WHERE status = $1 AND (project_id = $2 OR $2 = '') AND created_at >= $3 AND created_at <= $4
    `

	countEventDeliveries = `
    SELECT COUNT(id) FROM event_deliveries WHERE (project_id = ? OR ? = '') AND (event_id = ? OR ? = '') AND created_at >= ? AND created_at <= ?
    `

	updateEventDeliveriesStatus = `
    UPDATE event_deliveries SET status = ?, description = ?, updated_at = NOW() WHERE (project_id = ? OR ? = '')AND id IN (?);
    `

	updateEventDeliveryMetadata = `
    UPDATE event_deliveries SET status = $1, metadata = $2, latency_seconds = $3,  updated_at = NOW() WHERE id = $4 AND project_id = $5;
    `

	hardDeleteProjectEventDeliveries = `
    DELETE FROM event_deliveries WHERE project_id = $1 AND created_at >= $2 AND created_at <= $3;
    `
)
...
func (e *eventDeliveryRepo) DeleteProjectEventDeliveries(ctx context.Context, projectID string, filter *datastore.EventDeliveryFilter, _ bool) error {
