...
func (s *SubscriptionIntegrationTestSuite) Test_CreateSubscription_IncomingGroup() {
	group, err := testdb.SeedGroup(s.ConvoyApp.A.Store, s.ConvoyApp.A.Cache, uuid.NewString(), "test_group", "", datastore.IncomingGroup, nil)
	require.NoError(s.T(), err)
	...
	app, _ := testdb.SeedApplication(s.ConvoyApp.A.Store, s.ConvoyApp.A.Cache, group, uuid.NewString(), "", false)
	source, _ := testdb.SeedSource(s.ConvoyApp.A.Store, group, uuid.NewString(), "", "", nil)
	endpoint, _ := testdb.SeedEndpoint(s.ConvoyApp.A.Store, s.ConvoyApp.A.Cache, app, group.UID)
	bodyStr := fmt.Sprintf(`{
		"name": "sub-1",
		"type": "incoming",
		"app_id": "%s",
...
func (s *SubscriptionIntegrationTestSuite) Test_CreateSubscription_AppNotFound() {
	app, _ := testdb.SeedApplication(s.ConvoyApp.A.Store, s.ConvoyApp.A.Cache, &datastore.Group{UID: uuid.NewString()}, uuid.NewString(), "", false)
	endpoint, _ := testdb.SeedEndpoint(s.ConvoyApp.A.Store, s.ConvoyApp.A.Cache, app, s.DefaultGroup.UID)
	bodyStr := fmt.Sprintf(`{
		"name": "sub-1",
		"type": "incoming",
...
func (s *SubscriptionIntegrationTestSuite) Test_GetOneSubscription_IncomingGroup_ValidSubscription() {
	...
	group, err := testdb.SeedGroup(s.ConvoyApp.A.Store, s.ConvoyApp.A.Cache, uuid.NewString(), "test-group", "", datastore.IncomingGroup, nil)
	require.NoError(s.T(), err)
