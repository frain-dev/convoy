...
func (s *EventIntegrationTestSuite) Test_CreateFanoutEvent_MultipleEndpoints() {
	...
	// Just Before.
	_, _ = testdb.SeedEndpoint(s.ConvoyApp.A.Store, s.DefaultGroup, endpointID, "", ownerID, false, datastore.ActiveEndpointStatus)
	_, _ = testdb.SeedEndpoint(s.ConvoyApp.A.Store, s.DefaultGroup, uuid.NewString(), "", ownerID, false, datastore.ActiveEndpointStatus)
...
func (s *EventIntegrationTestSuite) Test_GetEndpointEvent_Valid_Event() {
	...
	// Just Before.
	endpoint, _ := testdb.SeedEndpoint(s.ConvoyApp.A.Store, s.DefaultGroup, uuid.NewString(), "", "", false, datastore.ActiveEndpointStatus)
...
func (s *EventIntegrationTestSuite) Test_ResendEventDelivery_Valid_Resend() {
	...
	// Just Before.
	endpoint, _ := testdb.SeedEndpoint(s.ConvoyApp.A.Store, s.DefaultGroup, uuid.NewString(), "", "", false, datastore.ActiveEndpointStatus)
	subscription, _ := testdb.SeedSubscription(s.ConvoyApp.A.Store, s.DefaultGroup, uuid.NewString(), datastore.OutgoingGroup, &datastore.Source{}, endpoint, &datastore.RetryConfiguration{}, &datastore.AlertConfiguration{}, &datastore.FilterConfiguration{})
...
func (s *EventIntegrationTestSuite) Test_GetEventsPaged() {
	...
	// Just Before.
	endpoint1, _ := testdb.SeedEndpoint(s.ConvoyApp.A.Store, s.DefaultGroup, uuid.NewString(), "", "", false, datastore.ActiveEndpointStatus)
	...
	endpoint2, _ := testdb.SeedEndpoint(s.ConvoyApp.A.Store, s.DefaultGroup, uuid.NewString(), "", "", false, datastore.ActiveEndpointStatus)
