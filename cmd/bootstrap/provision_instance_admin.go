...
func AddProvisionIACommand(a *cli.App) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(_ *cobra.Command, args []string) error {
			...
			instanceAdminUsers, err := postgres.NewOrgMemberRepo(a.DB).CountInstanceAdminUsers(context.Background())
			if err != nil {
				return err
			}

			if instanceAdminUsers > 0 {
				if token == "" {
			...
			} // else run it first time if zero IAs
...
func getInstanceAdmin(a *cli.App, token string) (*auth.AuthenticatedUser, *datastore.OrganisationMember, error) {
	err := initialize(a)
	if err != nil {
		return nil, nil, err
	}
	...
	m, err := orgMemberRepo.FetchInstanceAdminUserID(context.Background(), user.UID)
	if err != nil {
		if errors.Is(err, datastore.ErrOrgMemberNotFound) {
			return nil, nil, fmt.Errorf("instance admin user not found %w", err)
		}
		return nil, nil, err
	}
