...
func TestCircuitBreakersUpdate_Integration(t *testing.T) {
	...
	store := cb.NewRedisStore(app.Redis, clock.NewRealClock())
	...
	// Ensure the breaker state was cleared from Redis to pick new config
	got, getErr := app.Redis.Get(ctx, "breaker:cb-endpoint-1").Result()
