...
func TestFetchEarlyAdopterFeature_Interface(t *testing.T) {
	...
	require.Equal(t, fflag.ErrEarlyAdopterFeatureNotFound, err)
}
...
func TestAnyEnabledOverride(t *testing.T) {
	...
	exists, err := svc.AnyEnabledOverride(ctx, flag.UID)
	require.NoError(t, err)
	require.False(t, exists)

	require.NoError(t, svc.UpsertFeatureFlagOverride(ctx, &datastore.FeatureFlagOverride{
		FeatureFlagID: flag.UID,
		OwnerType:     "organisation",
	...
	exists, err = svc.AnyEnabledOverride(ctx, ulid.Make().String())
	require.NoError(t, err)
	require.False(t, exists)
}
