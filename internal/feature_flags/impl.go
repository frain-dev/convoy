...
func (s *Service) FetchEarlyAdopterFeature(ctx context.Context, orgID, featureKey string) (*fflag.EarlyAdopterFeatureInfo, error) {
	...
	if err != nil {
		if errors.Is(err, datastore.ErrEarlyAdopterFeatureNotFound) {
...
// AnyEnabledOverride implements fflag.FeatureFlagFetcher.
func (s *Service) AnyEnabledOverride(ctx context.Context, featureFlagID string) (bool, error) {
	exists, err := s.repo.AnyEnabledOverride(ctx, common.StringToPgText(featureFlagID))
	if err != nil {
		s.logger.Error("failed to check any enabled override", "error", err)
		return false, err
	}
	return exists, nil
}
