...
func TestGetPortalLinkByOwnerID_WithRefreshTokenAuthType_GeneratesAuthToken(t *testing.T) {
	db, ctx := setupTestDB(t)
	project := seedTestData(t, db)
	...
	// Fetch by owner ID - should provision a new auth token
	portalLink, err := service.GetPortalLinkByOwnerID(ctx, project.UID, ownerID)
...
func TestGetPortalLinkByOwnerID_WithStaticTokenAuthType_NoAuthKey(t *testing.T) {
	db, ctx := setupTestDB(t)
	project := seedTestData(t, db)
