...
UPDATE convoy.project_configurations SET
    ...
    ratelimit_duration = @ratelimit_duration,
    ...
    meta_events_event_type = @meta_events_event_type,
    meta_events_url = @meta_events_url,
    meta_events_secret = @meta_events_secret,
    meta_events_pub_sub = @meta_events_pub_sub,
    search_policy = @search_policy,
    ssl_enforce_secure_endpoints = @ssl_enforce_secure_endpoints,
    cb_sample_rate = @cb_sample_rate,
    cb_error_timeout = @cb_error_timeout,
    cb_failure_threshold = @cb_failure_threshold,
    cb_success_threshold = @cb_success_threshold,
    cb_observability_window = @cb_observability_window,
    cb_minimum_request_count = @cb_minimum_request_count,
    cb_consecutive_failure_threshold = @cb_consecutive_failure_threshold,
    updated_at = NOW()
...
VALUES (@id, @name, @type, @logo_url, @organisation_id, @project_configuration_id);
...
SELECT
    (SELECT EXISTS(SELECT 1 FROM convoy.subscriptions WHERE project_id = @project_id AND deleted_at IS NULL)) AS subscriptions_exist,
    (SELECT EXISTS(SELECT 1 FROM convoy.endpoints WHERE project_id = @project_id AND deleted_at IS NULL)) AS endpoints_exist,
    (SELECT EXISTS(SELECT 1 FROM convoy.sources WHERE project_id = @project_id AND deleted_at IS NULL)) AS sources_exist,
    (SELECT EXISTS(SELECT 1 FROM convoy.events WHERE project_id = @project_id AND deleted_at IS NULL)) AS events_exist;
...
WHERE e.created_at >= NOW() - MAKE_INTERVAL(hours := @interval_hours)
