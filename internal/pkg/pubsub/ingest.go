...
func (i *Ingest) handler(ctx context.Context, source *datastore.Source, msg string, metadata []byte) error {
	...
	if err = decoder.Decode(&convoyEvent); err != nil {
		log.WithError(err).Errorf("the payload for %s with id (%s) is badly formatted, please refer to the documentation or"+
			" use transfrom functions to properly format it (endpoint_id: %q, event_type: %q)", source.Name, source.UID, convoyEvent.EndpointID, convoyEvent.EventType)
		return err
	}
	...
	case "single":
		...
		if util.IsStringEmpty(ce.Params.EndpointID) {
			return fmt.Errorf("the payload with message type %s for %s with id (%s) doesn't include an endpoint id, please refer to the documentation or"+
				" use transfrom functions to properly format it (endpoint_id: %q, event_type: %q)", messageType, source.Name, source.UID, convoyEvent.EndpointID, convoyEvent.EventType)
		}
		...
		if err != nil {
			if errors.Is(err, datastore.ErrEndpointNotFound) {
				return fmt.Errorf("the payload for %s with id (%s) includes an invalid endpoint id: %q (event_type: %q)", source.Name, source.UID, ce.Params.EndpointID, convoyEvent.EventType)
			}
			return err
		}
