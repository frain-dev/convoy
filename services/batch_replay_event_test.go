	...
	tests := []struct {
	...
	}{
		{
			...
			dbFn: func(br *BatchReplayEventService) {
				...
				q.EXPECT().Write(ctx, convoy.CreateEventProcessor, convoy.CreateEventQueue, gomock.Any()).Times(2).Return(nil)
			},
			args: args{
				ctx: ctx,
				f: &datastore.Filter{
		...
		},
		{
			...
			dbFn: func(br *BatchReplayEventService) {
				...
				q, _ := br.Queue.(*mocks.MockQueuer)
				q.EXPECT().Write(ctx, convoy.CreateEventProcessor, convoy.CreateEventQueue, gomock.Any()).Times(2).Return(nil)
				q.EXPECT().Write(ctx, convoy.CreateEventProcessor, convoy.CreateEventQueue, gomock.Any()).Times(1).Return(errors.New("failed"))
			},
			args: args{
				ctx: ctx,
				f: &datastore.Filter{
