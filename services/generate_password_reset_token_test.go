	...
	tests := []struct {
	...
	}{
		{
			...
			dbFn: func(u *GeneratePasswordResetTokenService) {
				...
				q.EXPECT().Write(ctx, convoy.EmailProcessor, convoy.DefaultQueue, gomock.Any()).Times(1).Return(nil)
			},
		},
		{
		...
		},
		{
			...
			dbFn: func(u *GeneratePasswordResetTokenService) {
				...
				q, _ := u.Queue.(*mocks.MockQueuer)
				q.EXPECT().Write(ctx, convoy.EmailProcessor, convoy.DefaultQueue, gomock.Any()).Times(1).Return(errors.New("failed to write to queue"))
			},
			wantErr:    true,
