	...
	tests := []struct {
	...
	}{
		{
			...
			dbFn: func(es *EventService) {
				...
				q.EXPECT().WriteEventDelivery(gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any()).
					Times(2).Return(nil)
			},
			wantSuccesses: 2,
			wantFailures:  0,
	...
	tests := []struct {
	...
	}{
		{
			...
			dbFn: func(es *EventService) {
				...
				eq.EXPECT().WriteEventDelivery(gomock.Any(), convoy.TaskName("test_group-EventProcessor"), gomock.Any(), gomock.Any()).
					Times(1).Return(nil)
			},
		},
		{
