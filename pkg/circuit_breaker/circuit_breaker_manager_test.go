...
func TestCircuitBreakerManager_AddNewBreakerMidway(t *testing.T) {
	...
	b, err := NewCircuitBreakerManager(ClockOption(testClock), StoreOption(store), ConfigProviderOption(createTestConfigProvider(c)), LoggerOption(log.New("convoy", log.LevelInfo)))
	require.NoError(t, err)
...
func TestCircuitBreakerManager_ConsecutiveFailures(t *testing.T) {
	...
	b, err := NewCircuitBreakerManager(ClockOption(testClock), StoreOption(store), ConfigProviderOption(func(projectID string) *CircuitBreakerConfig { return c }), LoggerOption(log.New("convoy", log.LevelInfo)))
	require.NoError(t, err)
